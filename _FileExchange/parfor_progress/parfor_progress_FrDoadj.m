function percent = parfor_progress_FrDoadj(ExpShorthand,Prog_string,N)
...
%     end
    ...
    progress = fscanf(f, '%d');
    fclose(f);
%     numImg_processed = (length(progress)-1);
%     numImg_total = progress(1);
    
    percent = (length(progress)-1)/progress(1)*100;
    
    if nargout == 0
        fprintf('%s - %s: %3.f%% \n',ExpShorthand,Prog_string,percent)
