...
% draw data
figure;
patch(isosurface(V_ROI,0.5),'faceColor','b','LineStyle','none')
% plot3(XYZ(:,1),XYZ(:,2),XYZ(:,3), '.r' );
hold on;

%draw fit
mind = min( XYZ(:,1:3) )-100;
maxd = max( XYZ(:,1:3) )+100;
nsteps = 50;
...
figure;hMesh = mesh(x, y, z);
rotate(hMesh, [1 0 0], s.Orientation(2));
rotate(hMesh, [0 1 0], -s.Orientation(1));
rotate(hMesh, [0 0 1], -s.Orientation(3));
...
lighting phong;
 view(3)
  ...
  figure; patch(isosurface(V_ROI,0.5),'faceColor','b','LineStyle','none');hold on;
